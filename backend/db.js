...
-- (Ya no se usa: BINGOJY eliminó la sala de juego automática. Se deja la
...
// Migración incremental: cartas de regalo, consecutivas a las de venta normal.
// NULL = el sorteo no tiene regalos; si tiene valor, cualquier carta con
// grupo >= regalo_desde es de regalo (grupo < regalo_desde es normal).
if (!sorteosInfo.some((c) => c.name === 'regalo_desde')) {
  db.exec("ALTER TABLE sorteos ADD COLUMN regalo_desde INTEGER");
}
