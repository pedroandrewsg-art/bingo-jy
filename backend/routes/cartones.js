...
router.put('/asignar', requireAuth, requireAdmin, (req, res) => {
  ...
  if (asignables.length) {
    ...
    // Las cartas de regalo (grupo >= regalo_desde) son gratis: no suman al
    // monto de la venta ni, por lo tanto, a la recaudación/premio acumulado.
    const gruposNormales = new Set(
      asignables.filter((c) => !sorteo.regalo_desde || c.grupo < sorteo.regalo_desde).map((c) => c.grupo)
    );
  ...
  const asignados = [...new Set(asignables.map((c) => c[columna]))];
  const asignadosRegalo = sorteo.regalo_desde
    ? asignados.filter((n) => n >= sorteo.regalo_desde)
    : [];
...
router.get('/consulta', (req, res) => {
  ...
  res.json({
    ...
    esRegalo: !!sorteo.regalo_desde && numero >= sorteo.regalo_desde,
...
router.get('/consulta-nombre', (req, res) => {
  ...
  const resultados = jugadoresMatch.map((j) => {
    ...
    return {
      jugador_id: j.id,
      nombre: j.nombre,
      grupos: [...porGrupo.entries()].map(([grupo, cartones]) => ({
        grupo,
        esRegalo: !!sorteo.regalo_desde && grupo >= sorteo.regalo_desde,
        cartones,
      })),
    };
  });
