...
// settings.liberacion_pendientes_minutos, editable desde Configuración) ya
// venció, para que vuelvan a estar disponibles para otro jugador. '0'
// (default de fábrica) = desactivado, nunca libera solo.
//
...
// vuelve el cartón a 'disponible' y limpia dueño/marcados.
const db = require('./db');
...
function liberarVencidos(io) {
  ...
  const tx = db.transaction(() => {
    ...
    vencidos.forEach((c) => stmt.run(c.id));
  });
  tx();

  // Misma columna que usa el admin para identificar la carta en combos
  // (grupo) o el cartón suelto (numero) -- ver verificar-pago/liberar en
  // routes/cartones.js.
