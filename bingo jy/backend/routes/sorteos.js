...
function agruparPorConjunto(rows, regaloDesde) {
  const mapa = new Map();
  rows.forEach((r) => {
  ...
  return [...mapa.entries()]
    ...
    .map(([grupo, cards]) => ({
      ...
      esRegalo: regaloDesde != null && grupo >= regaloDesde,
    }));
}

// Separa conjuntos normales de los de regalo, cada sección conservando el
// orden ascendente que ya trae `agruparPorConjunto` — para listas
// compartibles (WhatsApp, "Verifícate en la Lista"), donde los normales van
// primero y los de regalo después.
function separarEnDosGrupos(conjuntos) {
  return [conjuntos.filter((g) => !g.esRegalo), conjuntos.filter((g) => g.esRegalo)];
}
function separarNormalesYRegalo(conjuntos) {
  const [normales, regalo] = separarEnDosGrupos(conjuntos);
  return [...normales, ...regalo];
}
...
router.get('/:id/lista-publica', (req, res) => {
  const sorteo = db.prepare('SELECT id, regalo_desde FROM sorteos WHERE id = ?').get(req.params.id);
  if (!sorteo) return res.status(404).json({ error: 'Sorteo no encontrado' });
  const conjuntos = separarNormalesYRegalo(
    agruparPorConjunto(cartonesConDueno(req.params.id), sorteo.regalo_desde).filter((g) => !g.disponible)
  );
  res.json({
    lista: conjuntos.map((g) => ({ etiqueta: etiquetaConjunto(g), nombre: g.nombre || '', pagado: g.pagado, esRegalo: g.esRegalo })),
  });
});
...
router.post('/', requireAuth, requireAdmin, (req, res) => {
  ...
  // BINGOJY ya solo juega con cartones personalizados (imágenes reales que
  ...
  const hastaNormal = parseInt(rango_hasta, 10);
  if (hastaNormal < desde) return res.status(400).json({ error: 'El rango es inválido' });
  // Cartas de regalo: consecutivas a las normales (rango_desde-rango_hasta
  // sigue siendo el span físico completo; regalo_desde marca desde dónde
  // empiezan las de regalo dentro de ese span).
  const cantidadRegalos = Math.max(0, parseInt(req.body.cantidad_regalos, 10) || 0);
  const hasta = hastaNormal + cantidadRegalos;
  const regaloDesde = cantidadRegalos > 0 ? hastaNormal + 1 : null;
  ...
  const info = db
    .prepare(
      `INSERT INTO sorteos (fecha_hora, rango_desde, rango_hasta, color, tipo_venta, costo, porcentaje_ganancia, modo_premio, patron, estatus, numeros_extraidos, encabezado, pie_pagina, catalogo_imagenes_id, regalo_desde)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'activo', '[]', ?, ?, ?, ?)`
    )
    .run(fecha_hora, desde, hasta, color, tipo_venta, costo, porcentaje_ganancia, modoPremio, figuras[0].patron, getSetting('default_encabezado'), getSetting('default_pie_pagina'), catalogoId, regaloDesde);
...
router.put('/:id/rango', requireAuth, requireAdmin, (req, res) => {
  ...
  const sorteo = db.prepare('SELECT * FROM sorteos WHERE id = ?').get(id);
  if (!sorteo) return res.status(404).json({ error: 'Sorteo no encontrado' });
  // `rango_hasta` sigue siendo la última carta NORMAL (misma convención que
  // al crear el sorteo); `cantidad_regalos` son las que se agregan después,
  // consecutivas. El span físico total (rango_hasta real en la tabla) es la
  // suma de ambos.
  const nuevoHastaNormal = parseInt(req.body.rango_hasta, 10);
  const nuevaCantidadRegalos = Math.max(0, parseInt(req.body.cantidad_regalos, 10) || 0);
  if (!Number.isFinite(nuevoHastaNormal) || nuevoHastaNormal < sorteo.rango_desde) {
    return res.status(400).json({ error: 'Rango inválido' });
  }
  const nuevoHasta = nuevoHastaNormal + nuevaCantidadRegalos;
  const nuevoRegaloDesde = nuevaCantidadRegalos > 0 ? nuevoHastaNormal + 1 : null;
  if (nuevoHasta === sorteo.rango_hasta && nuevoRegaloDesde === sorteo.regalo_desde) {
  ...
  } else if (nuevoHasta < sorteo.rango_hasta) {
  ...
  db.prepare('UPDATE sorteos SET rango_hasta = ?, regalo_desde = ? WHERE id = ?').run(nuevoHasta, nuevoRegaloDesde, id);
  const io = req.app.get('io');
  io.emit('sorteos-cambio', {});
  io.to(`sorteo-${id}`).emit('cartones-actualizados', { sorteoId: Number(id) });
  res.json({ sorteo: computeStats(id) });
...
router.get('/:id/vendidos-texto', requireAuth, requireAdmin, (req, res) => {
  ...
  const lineaConjunto = (g) => `${etiquetaConjunto(g)} - ${g.nombre || 'N/A'}${g.pagado ? ' ⭐' : ''}`;
  let text = `*BINGO ${sorteo.color.toUpperCase()} - VENDIDOS*\nFecha: ${sorteo.fecha_hora}\n\n`;
  text += normales.map(lineaConjunto).join('\n');
  if (regalo.length) text += `\n\n🎁 *CARTONES DE REGALO*\n` + regalo.map(lineaConjunto).join('\n');
  text += `\n\nTotal vendidos: ${normales.length + regalo.length}`;
  res.json({ texto: text });
});
