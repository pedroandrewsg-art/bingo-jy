...
// previa de una app externa, no participa del claro/oscuro de BINGOJY.
// ---------------------------------------------------------------------------
              ...
              {filtrada.map((r, i) => (
                <div key={i} className="flex items-center justify-between gap-2 bg-slate-800/50 rounded-lg px-3 py-1.5 text-sm">
                  ...
                  <span className="flex items-center gap-1.5 shrink-0">
                    {r.esRegalo && <Badge tone="rose">🎁 Regalo</Badge>}
      ...
      {resultado && (
        <div className="space-y-3">
          ...
          <div className="text-center">
            ...
            {resultado.esRegalo && <div className="mt-2 text-center"><Badge tone="rose">🎁 Regalo</Badge></div>}
          </div>
        ...
        <div className="space-y-3">
          ...
          {personaElegida.grupos.map((g) => (
            <div key={g.grupo} className="space-y-2">
              {g.esRegalo && <div className="text-center"><Badge tone="rose">🎁 Regalo</Badge></div>}
              {g.cartones.length > 1
    ...
    <div className="min-h-screen flex items-center justify-center p-4 relative">
      ...
      <div className={`w-full ${mode === 'consulta' ? 'max-w-2xl' : 'max-w-md'}`}>
        <div className="text-center mb-6">
          <img src={logoUrl || "logo.png"} alt="Bingo JY" className="w-24 h-24 mx-auto mb-2 rounded-full object-cover border-2 border-bingoaccent shadow-glow" />
          <h1 className="text-2xl font-black bg-gradient-to-r from-rose-300 to-pink-300 bg-clip-text text-transparent">Bingo JY</h1>
    ...
    <div className="min-h-screen flex">
      <aside className="w-60 shrink-0 bg-slate-950/60 border-r border-bingopurple/20 p-4 hidden md:flex md:flex-col gap-1">
        <div className="flex items-center gap-2 mb-6 px-2">
          <img src={logoUrl || "logo.png"} alt="Bingo JY" className="w-9 h-9 rounded-full object-cover shrink-0" />
          <span className="font-black text-rose-200 text-sm leading-tight">Bingo JY</span>
        </div>
        {tabs.map((t) => (
...
function AdminSorteos() {
  ...
  const emptyForm = { fecha_hora: '', rango_desde: 1, rango_hasta: 100, cantidad_regalos: 0, color: 'Verde', tipo_venta: 1, costo: 1, porcentaje_ganancia: 30, modo_premio: 'porcentaje', figuras: [], catalogo_imagenes_id: '' };
      ...
      {showForm && (
        <Modal title="Nuevo Sorteo" onClose={() => setShowForm(false)} wide>
          <form onSubmit={crearSorteo} className="grid md:grid-cols-2 gap-4">
            ...
            <div>
              <Label>Rango — Hasta N° (última carta normal)</Label>
              <Input required type="number" min="1" value={form.rango_hasta} onChange={(e) => setForm({ ...form, rango_hasta: e.target.value })} />
            </div>
            <div>
              <Label>🎁 Cartas de regalo (opcional, consecutivas)</Label>
              <Input type="number" min="0" value={form.cantidad_regalos} onChange={(e) => setForm({ ...form, cantidad_regalos: e.target.value })} placeholder="0" />
            </div>
            <div>
...
function SorteoDrawPanel({ sorteoId, onClose }) {
  ...
  const [regalosEdit, setRegalosEdit] = useState('');
  ...
  function iniciarEdicionRango() {
    setRangoError('');
    const hastaNormalActual = sorteo.regalo_desde != null ? sorteo.regalo_desde - 1 : sorteo.rango_hasta;
    const regalosActuales = sorteo.regalo_desde != null ? sorteo.rango_hasta - sorteo.regalo_desde + 1 : 0;
    setRangoHastaEdit(String(hastaNormalActual));
    setRegalosEdit(String(regalosActuales));
  ...
  async function guardarRango() {
    ...
    try {
      await apiFetch('/sorteos/' + sorteoId + '/rango', {
        method: 'PUT',
        body: JSON.stringify({ rango_hasta: Number(rangoHastaEdit), cantidad_regalos: Number(regalosEdit) || 0 }),
      });
      ...
      <Card>
        <div className="flex flex-wrap gap-6 justify-between items-center">
          ...
          <div>
            ...
            <div className="font-bold flex items-center gap-1.5">
              {sorteo.rango_desde}-{sorteo.regalo_desde != null ? sorteo.regalo_desde - 1 : sorteo.rango_hasta}
              <button type="button" onClick={iniciarEdicionRango} title="Ampliar o reducir la cantidad de cartas / regalos" className="text-slate-400 hover:text-rose-300 text-xs">✏️</button>
            </div>
          </div>
          {sorteo.regalo_desde != null && (
            <div>
              <div className="text-xs text-slate-400">🎁 Cartas de regalo</div>
              <div className="font-bold">{sorteo.regalo_desde}-{sorteo.rango_hasta}</div>
            </div>
          )}
        ...
        {editandoRango && (
          <div className="mt-3 pt-3 border-t border-slate-700/50 flex flex-wrap items-end gap-2">
            <div>
              <Label>Nueva última carta normal (hoy: {sorteo.rango_desde}-{sorteo.regalo_desde != null ? sorteo.regalo_desde - 1 : sorteo.rango_hasta})</Label>
              <Input type="number" min={sorteo.rango_desde} value={rangoHastaEdit} onChange={(e) => setRangoHastaEdit(e.target.value)} className="w-32" />
            </div>
            <div>
              <Label>🎁 Cartas de regalo</Label>
              <Input type="number" min="0" value={regalosEdit} onChange={(e) => setRegalosEdit(e.target.value)} className="w-32" />
            </div>
            ...
            <p className="w-full text-xs text-slate-500">
              Un número más alto agrega cartas nuevas. Uno más bajo las quita — solo funciona si esas cartas todavía están disponibles (no apartadas ni vendidas). Las cartas de regalo son consecutivas, justo después de la última normal.
            </p>
          </div>
        )}
        {editandoCatalogo && (
        ...
        {(
          <div className="space-y-2 max-h-[32rem] overflow-y-auto pr-1 -mr-1">
            {[...cartonesPorGrupo.entries()].sort((a, b) => a[0] - b[0]).map(([grupo, cards]) => {
              ...
              const esRegalo = sorteo.regalo_desde != null && grupo >= sorteo.regalo_desde;
              return (
...
// admin fija cuántos minutos esperar desde que un cartón queda apartado
// (estado 'vendido', sin pago verificado) antes de devolverlo solo a
    ...
    <Card className="space-y-3 max-w-xl">
      <div>
        ...
        <p className="text-xs text-slate-500 mt-1">
          Cuando un cartón queda apartado ("vendido", pago sin confirmar) y pasan estos minutos sin que vos confirmes el pago, el cartón vuelve solo a "disponible" para que otro lo pueda comprar. Dejá en 0 para desactivarlo (nunca libera solo, como hasta ahora).
        </p>
      </div>
      {!cargando && (
...
// mirror de disponibles-texto/pendientes-texto/lista-texto en
...
function WhatsappLivePanel({ sorteoId }) {
  ...
  function textoTodo() {
    ...
    const linea = (g) => {
      ...
      const prefijoRegalo = g.esRegalo ? ` ${config.regalo_emoji}` : '';
    ...
    const regalo = conjuntos.filter((g) => g.esRegalo);
    let cuerpo = normales.map(linea).join('\n');
    if (regalo.length) cuerpo += `\n\n${config.regalo_emoji} *${config.regalo_subtitulo}*\n` + regalo.map(linea).join('\n');
  ...
  function textoLibres() {
    ...
    const normales = disponibles.filter((g) => !g.esRegalo);
    const regalo = disponibles.filter((g) => g.esRegalo);
    ...
    texto += normales.length ? normales.map((g) => g.etiquetaEmoji).join('\n') : 'No quedan cartones normales disponibles.';
    if (regalo.length) texto += `\n\n${config.regalo_emoji} *${config.regalo_subtitulo}*\n` + regalo.map((g) => `${g.etiquetaEmoji} ${config.regalo_emoji}`).join('\n');
    if (total) texto += `\n\n${config.disponibles_pie}`;
    return texto.trim();
  }

  // mirror de pendientes-texto. Los cartones de regalo NUNCA aparecen acá —
  // son gratis, no generan deuda; solo los normales pueden estar pendientes de pago.
  ...
  const totalPendientes = conjuntos.filter((g) => !g.esRegalo && !g.disponible && !g.pagado).length;
    ...
    <Card className="space-y-3">
      ...
      {(!datos || !config) ? <Spinner /> : (
        <>
          ...
          <WhatsappChatFrame titulo="Bingo JY" subtitulo="En línea" logoUrl={logoUrl}>
            {subTab === 'nombres' ? (
              <div className="space-y-2">
                ...
                <div className="max-h-72 overflow-y-auto space-y-1.5 pr-1">
                  ...
                  {resultadosNombres.map((g) => (
                    <div key={g.grupo} className="flex items-center justify-between gap-2 bg-white/80 rounded-lg px-3 py-1.5 text-sm">
                      ...
                      <span className="flex items-center gap-1.5 shrink-0">
                        {g.esRegalo && <Badge tone="rose">{config.regalo_emoji} Regalo</Badge>}
        ...
        {!configMinimizada && (!config ? <Spinner /> : (
          <div className="space-y-4 mt-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>Subtítulo — Regalo</Label>
                <Input value={config.regalo_subtitulo} onChange={(e) => setCampo('regalo_subtitulo', e.target.value)} />
              </div>
              <div>
                <Label>Emoji — Regalo</Label>
